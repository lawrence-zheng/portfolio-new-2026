...
(() => {
  ...
  function createPseudoSheet(doc) {
    ...
    return (pseudo, css) => {
      ...
      const cssImp = pseudo === "before" || pseudo === "after" ? css : css.split(";").map((d) => d.trim()).filter(Boolean).map((d) => d.toLowerCase().includes("!important") ? d : d + " !important").join("; ");
      el.sheet.insertRule(sel + "{" + cssImp + "}", el.sheet.cssRules.length);
      cache.set(k, cls);
      return cls;
    };
  }
